...
export function OnboardingChecklistModal({ open, items, onDismiss, onSilenceAll, onGoToTarget }: OnboardingChecklistModalProps) {
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <div
      className={[
        'pointer-events-auto fixed bottom-4 right-4 flex w-[min(24rem,calc(100vw-2rem))] flex-col gap-4 rounded-2xl border px-5 py-4 text-sm leading-6 shadow-lg',
    ...
    >
      ...
      <button
        ...
        className="self-start text-[11.5px] font-medium text-cyan-700/60 underline-offset-2 transition hover:text-cyan-900 hover:underline dark:text-cyan-100/60 dark:hover:text-white"
      >
        Não ver mais guias
      </button>
    </div>
