...
export function HomeHeader({ onOpenLogin, onOpenRegister }: HomeHeaderProps) {
  ...
  return (
    <header className="sticky inset-x-0 top-0 z-50 border-b border-[rgba(30,196,220,0.12)] bg-[#03161D]/88 backdrop-blur-xl">
      ...
      <div className="mx-auto flex h-[76px] max-w-[1760px] items-center justify-between gap-5 px-5 sm:px-8 xl:px-10">
        ...
        <nav className="hidden items-center gap-9 lg:flex" aria-label="Navegação principal da Home">
          {NAV_LINKS.map(({ label, to }) => (
            <Link
              key={to}
              to={to}
              className="rounded-lg px-1 py-2 text-[15px] font-medium text-site-textSub outline-none transition hover:text-site-text focus-visible:ring-2 focus-visible:ring-site-accent/70 focus-visible:ring-offset-4 focus-visible:ring-offset-[#03161D] motion-reduce:transition-none"
            >
              {label}
            </Link>
          ))}
          ...
          <nav className="grid gap-1" aria-label="Navegação principal móvel">
            {NAV_LINKS.map(({ label, to }) => (
              <Link
                ...
                to={to}
                onClick={closeMenu}
                className="min-h-11 rounded-xl px-3 py-3 text-[14px] font-medium text-site-textSub outline-none transition hover:bg-[rgba(14,196,216,0.08)] hover:text-site-text focus-visible:ring-2 focus-visible:ring-site-accent/70 motion-reduce:transition-none"
              >
                {label}
              </Link>
            ))}
